...
public class DatabaseTransactions extends SQLiteOpenHelper {
    ...
    public JSONObject getMaxHighest(String userId) throws JSONException{
        ...
        Cursor res5;
        res5 = db.rawQuery("select sum(tw) as tw, (substr(strftime(\"%Y\",start_date),3, 2) || strftime(\"/%m/%d\",start_date)) as start_date, substr(strftime('%Y',end_date),3, 2) || strftime('/%m/%d',end_date) as end_date, group_concat(DISTINCT station_no) as station_no, muscle_type, term_taxonomy_id from (select sum(tw) as tw, start_date, end_date, group_concat(DISTINCT station_no) as station_no , muscle_type, object_id, term_taxonomy_id  from (select start_date, end_date, d.muscle_type, d.user_id, max(d.total_weight) as tw, d.date, d.station_no from tbl_year_weeks y JOIN dailyrecord d ON d.date BETWEEN y.start_date AND y.end_date WHERE d.user_id =  ? group by date, station_no) A JOIN `wp_term_relationships` wtr ON wtr.object_id = muscle_type group by date, muscle_type) B group by end_date,term_taxonomy_id order by tw desc LIMIT 1", new String[]{userId});
        res5.moveToFirst();
        while (res5.isAfterLast() == false) {
            if ((res5 != null) && (res5.getCount() > 0)) {

                maxStationMaxEffortmg7 = res5.getString(res5.getColumnIndex("start_date")) + "- "  + res5.getString(res5.getColumnIndex("end_date"));
    ...
    private String get_total_reps_by_muscle_type(String user_id, String machineno, String last_date, String set_number, String station_no, String muscle_id) {
        ...
        Cursor res;
        res = db.rawQuery("select total_reps from  dailyrecord where user_id = ? and station_no = ? and muscle_type = ? and date = ? and set_number = ? order by id desc", new String[]{user_id,muscle_id,last_date,set_number});
        res.moveToFirst();
        while (res.isAfterLast() == false) {
            if ((res != null) && (res.getCount() > 0)) {
    ...
    public JSONObject getMaxWeightByMuscle(String userid, String muscleName, String station_no) throws JSONException{
        ...
        Cursor res;
        res = db.rawQuery("select * from dailyrecord where Weight = (select MAX(Weight) from dailyrecord where user_id=? and muscle_type = ?) and user_id=? and muscle_type = ?", new String[]{userid,muscle_id,userid,muscle_id});
        res.moveToFirst();
        while (res.isAfterLast() == false) {
            if ((res != null) && (res.getCount() > 0)) {
        ...
        Cursor res1;
        res1 = db.rawQuery("select * from dailyrecord where total_weight  = (select MAX(total_weight) from dailyrecord where user_id= ? and muscle_type = ?) and user_id=? and muscle_type = ?", new String[]{userid,muscle_id,userid,muscle_id});
        res1.moveToFirst();
        while (res1.isAfterLast() == false) {
            if ((res1 != null) && (res1.getCount() > 0)) {
        ...
        while (res3.isAfterLast() == false) {
            if ((res3 != null) && (res3.getCount() > 0)) {
                maxEffort24 = res3.getString(res3.getColumnIndex("tw"));
                maxEffort24StationNo = res3.getString(res3.getColumnIndex("station_no")).replace(",", " - ");
            }
            res3.moveToNext();
        }
        res3.close();
        ...
        values1.put("muValueMaxEffort_24", maxWeight);
    ...
    public String syncLocalDB(String userId, JSONArray records, JSONArray station, JSONArray yearDays, JSONArray yearWeeks) {
